...
let primary = new peg.Token('bool literal').apply(tok => new ast.BoolLiteral(tok.bool)).choice(new peg.Token("'nil'").apply(tok => new ast.NilLiteral())).choice(new peg.Token('number literal').apply(tok => new ast.NumberLiteral(tok.num))).choice(new peg.Token('string literal').apply(tok => new ast.StringLiteral(tok.str))).choice(new peg.Token("identifier").apply(ident => new ast.VarExpr(ident.name))).choice(new peg.Token("'('").chain(new peg.Indirect(expression_indirect)).chain(new peg.Token("')'")).apply(([[oparen, expr], cparen]) => expr));
